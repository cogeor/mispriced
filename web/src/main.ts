import './style.css';
import type { DashboardData, ScatterPoint, BacktestItem } from './types';
import { aggregateToSummary } from './utils';
...
let mispricingMode: 'raw' | 'sizeNeutral' = 'raw';
let colorBy: 'sector' | 'mispricing' = 'sector';
...
function recomputeDecay(ts: BacktestItem[], metric: string) {
...
function renderSignalQualityTable(sectorStats: BacktestItem[], indexStats: BacktestItem[], metric: string) {
...
function renderTopTables(data: ScatterPoint[], key: string) {
    const sorted = [...data].sort((a, b) => (a as any)[key] - (b as any)[key]);
...
function renderTableRows(id: string, rows: ScatterPoint[], key: string) {
    ...
    el.innerHTML = rows.map(r => `
        <tr class="border-b border-gray-800 hover:bg-gray-800/50 transition-colors cursor-pointer" onclick="window.open('https://www.google.com/finance/quote/${r.ticker}:NYSE', '_blank')">
            ...
            <td class="py-3 text-right ${(r as any)[key] < 0 ? 'text-green-400' : 'text-red-400'} font-bold">
                ${((r as any)[key] * 100).toFixed(1)}%
            </td>
...
function setMode(mode: 'raw' | 'sizeNeutral') {
    mispricingMode = mode;
...
function setColor(c: 'sector' | 'mispricing') {
    colorBy = c;
